...
import random #imports the random module to generate random numbrs from a given distribution 
    ...
    while menu == menu_new:
        ...
        while (p1_posc <= 24) or (p2_posc <= 24) and menu == menu_new:
            if p1_dicec == p2_dicec:
                p1_roll = input("Cow, Roll the dice: ")
            ...
            if p2_dicec < p1_dicec:
                p2_roll = input("Bull, Roll the dice: ")
                ...
                if p2_posc < 24:
                    ...
                    break
                p2_dicec += 1
#prints the leaderboard which contains the losses and wins of each player
...
#prints a goodbye message and proceeds to exit the game for the player
    if menu == menu_end:
        print("Goodbye, Come back when you can ;)")
        exit()
